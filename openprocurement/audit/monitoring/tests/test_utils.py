...
class GetAccessTokenTests(unittest.TestCase):
    def test_token_query_param(self):
        self.assertEqual(get_access_token(request=mock.Mock(params={"acc_token": "test_token"})), "test_token")

    def test_token_headers(self):
        self.assertEqual(
            get_access_token(request=mock.Mock(params={}, headers={"X-Access-Token": "test_token"})), "test_token"
        )
    ...
    def test_token_body(self):
        request = mock.Mock(
            ...
            params={},
            headers={},
            json_body={"access": {"token": "test_token"}},
        )
        self.assertEqual(get_access_token(request=request), "test_token")

    def test_no_token(self):
        request = mock.Mock(method="POST", content_type="application/json", params={}, headers={}, json_body={})
