...
def get_file(request):
    ...
    request.response.content_disposition = build_header(document.title, filename_compat=quote(document.title)).decode(
        "utf-8"
    )
...
def append_revision(request, obj, patch):
    revision_data = {"author": request.authenticated_userid, "changes": patch, "rev": obj.rev}
...
def fix_url(item, app_url):
    if isinstance(item, list):
        ...
        if "format" in item and "url" in item and "?download=" in item["url"]:
            ...
            return [fix_url(item[i], app_url) for i in item if isinstance(item[i], dict) or isinstance(item[i], list)]
