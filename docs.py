...
from openprocurement.audit.monitoring.tests.base import BaseWebTest as MonitoringWebTest
from openprocurement.audit.monitoring.tests.base import DSWebTestMixin
        ...
        if hasattr(self, "file_obj") and not self.file_obj.closed:
            ...
            if req.body:
                try:
                    ...
                    self.file_obj.write("DATA:\n" + json.dumps(obj, indent=self.indent, ensure_ascii=self.ensure_ascii))
                self.file_obj.write("\n")
            self.file_obj.write("\n")

    def write_response(self, resp):
        if hasattr(self, "file_obj") and not self.file_obj.closed:
            headers = [(n.title(), v) for n, v in resp.headerlist if n.lower() != "content-length"]
            headers.sort()
            self.file_obj.write(
                str("Response: %s\n%s\n")
                % (
            ...
            if resp.testbody:
                try:
                    ...
                    self.file_obj.write(json.dumps(obj, indent=self.indent, ensure_ascii=self.ensure_ascii))
                    self.file_obj.write("\n")
            self.file_obj.write("\n")
...
class OptionsResourceTest(BaseMonitoringWebTest):
    ...
    def test_monitoring_list_options_query_params(self, mock_restricted):
        ...
        with open(
            "docs/source/monitoring/feed/http/monitorings-with-options-query-params.http", "wt"
...
class MonitoringsResourceTest(BaseMonitoringWebTest, DSWebTestMixin):
    def setUp(self):
        ...
        self.party = {
            ...
            "contactPoint": {"name": "Oleksii Kovalenko", "telephone": "0440000000"},
    ...
    def test_monitoring_life_cycle_with_violations(self, mock_api_client, mock_restricted):
        ...
        with freeze_time("2018.01.02 00:00"):
            with open(
                "docs/source/monitoring/tutorial/http/monitoring-publish-wo-decision.http", "wt"
            ) as self.app.file_obj:
                self.app.patch_json("/monitorings/{}".format(monitoring_id), {"data": {"status": "active"}}, status=422)

        # PUBLISH

        with freeze_time("2018.01.02 01:05"):
            with open(
                "docs/source/monitoring/tutorial/http/monitoring-publish-first-step.http", "wt"
            ) as self.app.file_obj:
                self.app.patch_json(
        ...
        with freeze_time("2018.01.02 01:10"):
            with open(
                "docs/source/monitoring/tutorial/http/monitoring-publish-add-document.http", "wt"
            ) as self.app.file_obj:
                self.app.post_json(
        ...
        with freeze_time("2018.01.02 01:15"):
            with open(
                "docs/source/monitoring/tutorial/http/monitoring-publish-second-step.http", "wt"
            ) as self.app.file_obj:
                self.app.patch_json("/monitorings/{}".format(monitoring_id), {"data": {"status": "active"}}, status=200)
        ...
        with freeze_time("2018.01.04 00:00"):
            with open("docs/source/monitoring/tutorial/http/dialogue-get-credentials.http", "wt") as self.app.file_obj:
                response = self.app.patch_json(
                    "/monitorings/{}/credentials?acc_token={}".format(monitoring_id, tender_token), status=200
                )
        ...
        with open("docs/source/monitoring/tutorial/http/post-get-documents.http", "wt") as self.app.file_obj:
            self.app.get("/monitorings/{}/posts/{}/documents".format(monitoring_id, post_id), status=200)
        ...
        with freeze_time("2018.01.04 00:05"):
            with open("docs/source/monitoring/tutorial/http/post-answer.http", "wt") as self.app.file_obj:
                response = self.app.post_json(
                    ...
                    {"data": {"title": "Sit amet", "description": "Dolor sit amet", "relatedPost": post_id}},
                    status=201,
                )
        ...
        with open("docs/source/monitoring/tutorial/http/posts-get.http", "wt") as self.app.file_obj:
            self.app.get("/monitorings/{}/posts".format(monitoring_id), status=200)
        ...
        with freeze_time("2018.01.05 00:00"):
            with open("docs/source/monitoring/tutorial/http/conclusion-wo-violations.http", "wt") as self.app.file_obj:
                response = self.app.patch_json(
                    "/monitorings/{}".format(monitoring_id),
                    {"data": {"conclusion": {"violationOccurred": False, "relatedParty": party_id}}},
                )
        ...
        with freeze_time("2018.01.05 00:10"):
            with open(
                "docs/source/monitoring/tutorial/http/conclusion-failed-required.http", "wt"
            ) as self.app.file_obj:
                response = self.app.patch_json(
                    "/monitorings/{}".format(monitoring_id),
                    {
        ...
        with freeze_time("2018.01.05 00:17"):
            with open(
                "docs/source/monitoring/tutorial/http/conclusion-other-validation.http", "wt"
            ) as self.app.file_obj:
                response = self.app.patch_json(
                    "/monitorings/{}".format(monitoring_id),
                    {
                ...
                self.assertEqual(
                    ...
                    [
                        {
                            "description": {"otherViolationType": ["This field is required."]},
                            "location": "body",
                            "name": "conclusion",
                        }
                    ],
        ...
        with freeze_time("2018.01.03 00:10"):
            with open(
                "docs/source/monitoring/tutorial/http/post-conclusion-add-document.http", "wt"
            ) as self.app.file_obj:
                self.app.post_json(
                    "/monitorings/{}/posts/{}/documents?acc_token={}".format(
        ...
        with freeze_time("2018.01.06 07:30"):
            with open("docs/source/monitoring/tutorial/http/appeal-post-again.http", "wt") as self.app.file_obj:
                self.app.put_json(
                    "/monitorings/{}/appeal?acc_token={}".format(monitoring_id, tender_owner_token),
                    {"data": {"description": "Addition to the appeal description", "documents": [another_document]}},
                    status=403,
                )
        ...
        with freeze_time("2018.01.06 07:15"):
            with open(
                "docs/source/monitoring/tutorial/http/add-proceeding-to-appeal-again.http", "wt"
            ) as self.app.file_obj:
                self.app.patch_json(
                    "/monitorings/{}/appeal?acc_token={}".format(monitoring_id, tender_owner_token),
                    {
            ...
            with open("docs/source/monitoring/tutorial/http/appeal-patch-doc.http", "wt") as self.app.file_obj:
                self.app.patch_json(
                    "/monitorings/{}/appeal/documents/{}?acc_token={}".format(
                        monitoring_id, appeal_doc_id, tender_owner_token
                    ),
                    {
                        "data": {
                            "title": "letter(0).doc",
        ...
        with freeze_time("2018.01.09 00:00"):
            with open(
                "docs/source/monitoring/tutorial/http/elimination-resolution-post.http", "wt"
            ) as self.app.file_obj:
                self.app.patch_json(
                    "/monitorings/{}".format(monitoring_id),
                    {
        ...
        with freeze_time("2018.01.09 01:30"):
            with open(
                "docs/source/monitoring/tutorial/http/add-proceeding-to-liability.http", "wt"
            ) as self.app.file_obj:
                self.app.patch_json(
        ...
        with freeze_time("2018.01.09 02:00"):
            with open(
                "docs/source/monitoring/tutorial/http/add-proceeding-to-liability-again.http", "wt"
            ) as self.app.file_obj:
                self.app.patch_json(
                    "/monitorings/{}/liabilities/{}".format(monitoring_id, liability_id),
                    {
        ...
        with freeze_time("2018.01.25 01:32"):
            with open("docs/source/monitoring/tutorial/http/monitoring-documents-get.http", "wt") as self.app.file_obj:
                self.app.get("/monitorings/{}/documents/{}".format(monitoring_id, doc_id), status=200)

        with freeze_time("2018.01.25 01:35"):
            with open(
                "docs/source/monitoring/tutorial/http/monitoring-documents-patch.http", "wt"
            ) as self.app.file_obj:
                self.app.patch_json(
                    "/monitorings/{}/documents/{}".format(monitoring_id, doc_id),
                    {
        ...
        with freeze_time("2018.01.25 01:40"):
            with open(
                "docs/source/monitoring/tutorial/http/monitoring-documents-get-collection.http", "wt"
            ) as self.app.file_obj:
                self.app.get("/monitorings/{}/documents".format(monitoring_id), status=200)
    ...
    def test_monitoring_life_cycle_with_no_violations(self, mock_restricted):
        ...
        with freeze_time("2018.01.04 00:00"):
            self.app.patch_json(
                "/monitorings/{}".format(monitoring_id),
                {"data": {"conclusion": {"violationOccurred": False, "relatedParty": party_id}, "status": "declined"}},
            )
        ...
        with freeze_time("2018.01.03 00:00"):
            with open("docs/source/monitoring/tutorial/http/monitoring-to-stopped.http", "wt") as self.app.file_obj:
                self.app.patch_json(
                    ...
                    {
                        "data": {
                            "cancellation": {"description": "Complaint was created", "relatedParty": party_id},
                            "status": "stopped",
                        }
                    },
                )
        ...
        with freeze_time("2018.01.03 00:00"):
            with open("docs/source/monitoring/tutorial/http/monitoring-to-cancelled.http", "wt") as self.app.file_obj:
                self.app.patch_json(
                    ...
                    {
                        "data": {
                            "cancellation": {"description": "Some reason", "relatedParty": party_id},
                            "status": "cancelled",
                        }
                    },
                )
    ...
    def test_monitoring_life_cycle_reactivation(self, mock_restricted):
        ...
        with freeze_time("2025.11.11 10:00"):
            with open(
                "docs/source/monitoring/tutorial/http/stopped-monitoring-to-active.http", "wt"
            ) as self.app.file_obj:
                self.app.patch_json(
                    "/monitorings/{}".format(monitoring_id),
                    {
            ...
            with open(
                "docs/source/monitoring/tutorial/http/active-monitoring-to-stopped-second-time.http", "wt"
            ) as self.app.file_obj:
                self.app.patch_json(
                    "/monitorings/{}".format(monitoring_id),
                    {
...
class InspectionsByMonitoringResourceTest(BaseInspectionWebTest):
    ...
    def test_restricted_inspection(self, mock_restricted):
        with freeze_time("2018.01.01 00:00"):
            with open(
                "docs/source/inspection/tutorial/http/post-inspection-restricted-true.http", "wt"
            ) as self.app.file_obj:
                response = self.app.post_json(
                    "/inspections",
                    {
        ...
        with open(
            "docs/source/inspection/tutorial/http/get-inspection-restricted-true-sas.http", "wt"
        ) as self.app.file_obj:
        ...
        with open(
            "docs/source/inspection/tutorial/http/get-inspection-restricted-true-brokerr.http", "wt"
        ) as self.app.file_obj:
            self.app.get("/inspections/{}".format(inspection_id), status=200)

        self.app.authorization = ("Basic", (self.broker_name, self.broker_pass))
        with open(
            "docs/source/inspection/tutorial/http/get-inspection-restricted-true-broker.http", "wt"
        ) as self.app.file_obj:
            self.app.get("/inspections/{}".format(inspection_id), status=200)
    ...
    def test_tutorial(self):
        ...
        with freeze_time("2018.01.01 00:01"):
            with open("docs/source/request/tutorial/http/request-documents-get.http", "wt") as self.app.file_obj:
                response = self.app.get("/requests/{}/documents".format(request_id))
        self.assertEqual(response.status, "200 OK")
        ...
        with open("docs/source/request/tutorial/http/requests-list.http", "wt") as self.app.file_obj:
            response = self.app.get("/requests?opt_fields=answer")

        with open("docs/source/request/tutorial/http/requests-list-answered.http", "wt") as self.app.file_obj:
            response = self.app.get("/requests?mode=answered&opt_fields=answer")
...
class MonitoringRestrictedResourceTest(BaseMonitoringWebTest, DSWebTestMixin):
    ...
    def test_monitoring_restricted(self, mock_restricted, mock_api_client):
        ...
        self.app.authorization = ("Basic", (self.sas_name, self.sas_pass))
        with open(
            "docs/source/monitoring/tutorial/http/post-monitoring-restricted-true.http", "wt"
        ) as self.app.file_obj:
            response = self.app.post_json(
                "/monitorings",
                {
        ...
        with freeze_time("2018.01.06 00:00"):
            response = self.app.patch_json(
                "/monitorings/{}".format(monitoring_id),
                {"data": {"conclusion": conclusion, "status": "addressed"}},
                status=200,
            )
        ...
        with freeze_time("2018.01.25 01:40"):
            with open(
                "docs/source/monitoring/tutorial/http/get-monitoring-restricted-true-sas.http", "wt"
            ) as self.app.file_obj:
                self.app.get("/monitorings/{}".format(monitoring_id), status=200)

        self.app.authorization = ("Basic", (self.broker_name_r, self.broker_pass_r))
        with freeze_time("2018.01.25 01:40"):
            with open(
                "docs/source/monitoring/tutorial/http/get-monitoring-restricted-true-brokerr.http", "wt"
            ) as self.app.file_obj:
                self.app.get("/monitorings/{}".format(monitoring_id), status=200)
        ...
        with freeze_time("2018.01.25 01:40"):
            with open(
                "docs/source/monitoring/tutorial/http/get-monitoring-restricted-true-broker.http", "wt"
            ) as self.app.file_obj:
                self.app.get("/monitorings/{}".format(monitoring_id), status=200)
