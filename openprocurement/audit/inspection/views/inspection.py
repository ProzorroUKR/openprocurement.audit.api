...
class InspectionsResource(RestrictedResourceListingMixin, MongodbResourceListing):
    ...
    @json_view(content_type="application/json", permission="create_inspection", validators=(validate_inspection_data,))
    def post(self):
        ...
        save_inspection(self.request, insert=True)
        LOGGER.info(
            "Created inspection {}".format(inspection.id),
            extra=context_unpack(self.request, {"MESSAGE_ID": "inspection_create"}, {"MONITORING_ID": inspection.id}),
        )
        self.request.response.status = 201
        self.request.response.headers["Location"] = self.request.route_url("Inspection", inspection_id=inspection.id)
...
class InspectionResource(APIResource):
    ...
    @json_view(
        content_type="application/json", validators=(validate_patch_inspection_data,), permission="edit_inspection"
    )
    def patch(self):
        inspection = self.request.validated["inspection"]
        apply_patch(self.request, src=self.request.validated["inspection_src"])
        LOGGER.info(
            "Updated inspection {}".format(inspection.id),
