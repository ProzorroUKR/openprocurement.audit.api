...
class MonitoringLiabilityResourceTest(BaseLiabilityTest):
    def test_fail_patch_liability_before_added(self):
        ...
        self.assertEqual(
            response.json["errors"], [{"location": "url", "name": "liability_id", "description": "Not Found"}]
        )
...
class MonitoringLiabilityPostedResourceTest(BaseLiabilityTest):
    ...
    def test_get_liability(self):
        self.app.authorization = ("Basic", (self.sas_name, self.sas_pass))
        response = self.app.get("/monitorings/{}/liabilities/{}".format(self.monitoring_id, self.liability_id))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content_type, "application/json")
    ...
    def test_success_post_document(self):
        self.app.authorization = ("Basic", (self.sas_name, self.sas_pass))
        response = self.app.post_json(
            "/monitorings/{}/liabilities/{}/documents".format(self.monitoring_id, self.liability_id),
            {
                "data": {
                    "title": "lorem.doc",
                    "url": self.generate_docservice_url(),
