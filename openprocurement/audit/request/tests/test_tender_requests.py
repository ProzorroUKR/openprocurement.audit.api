...
class TenderRequestsResourceTest(BaseWebTest):
    ...
    def test_get_custom_fields(self):
        ...
        self.app.authorization = ("Basic", (self.sas_name, self.sas_pass))
        response = self.app.get("/tenders/{}/requests?opt_fields=parties%2Cdocuments".format(tender_id))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content_type, "application/json")
        self.assertEqual([e["id"] for e in response.json["data"]], ids)
        self.assertEqual(
    ...
    def test_get_with_pagination(self):
        ...
        response = self.app.get("/tenders/{}/requests?opt_fields=description&limit=2&page=2".format(tender_id))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content_type, "application/json")
    ...
    def test_get_with_pagination_not_full_page(self):
        ...
        response = self.app.get("/tenders/{}/requests?opt_fields=description&limit=2&page=3".format(tender_id))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content_type, "application/json")
        self.assertEqual(response.json["total"], 5)
    ...
    def test_get_with_pagination_out_of_bounds_page(self):
        ...
        response = self.app.get("/tenders/{}/requests?opt_fields=description&limit=2&page=4".format(tender_id))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content_type, "application/json")
        self.assertEqual(response.json["total"], 5)
