...
class MonitoringResourceTest(BaseWebTest):
    ...
    def test_patch_forbidden_url(self):
        self.app.patch_json("/monitorings/{}".format(self.monitoring_id), {"status": "active"}, status=403)

    def test_patch_without_decision(self):
        self.app.authorization = ("Basic", (self.sas_name, self.sas_pass))
        response = self.app.patch_json(
            "/monitorings/{}".format(self.monitoring_id), {"data": {"status": "active"}}, status=422
        )
    ...
    def test_patch_nothing(self):
        ...
        response = self.app.patch_json(
            "/monitorings/{}".format(self.monitoring_id), {"data": {"procuringStages": ["planning"]}}
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content_type, "application/json")
    ...
    def test_patch_to_active(self):
        ...
        now_date = datetime.now(TZ)
        if accelerator:
            end_date = calc_datetime(now_date, MONITORING_TIME, accelerator=accelerator)
        else:
            end_date = calc_working_datetime(now_date, MONITORING_TIME, midnight=True, calendar=WORKING_DAYS)
        response = self.app.patch_json(
            "/monitorings/{}".format(self.monitoring_id),
            {
                "data": {
                    "status": "active",
                    "decision": {"description": "text", "date": (now_date - timedelta(days=2)).isoformat()},
                }
            },
        )
    ...
    def test_patch_risk_indicators_forbidden(self):
        ...
        self.app.patch_json(
            "/monitorings/{}".format(self.monitoring_id),
            {"data": {"riskIndicators": ["some_new_ids", "fooBar"]}},
            status=422,
        )
    ...
    def test_patch_to_active_already_in_active(self):
        ...
        response = self.app.patch_json(
            "/monitorings/{}".format(self.monitoring_id),
            {
                "data": {
                    "status": "active",
                    "decision": {"description": "text", "date": (datetime.now() + timedelta(days=2)).isoformat()},
                }
            },
        )
        self.assertEqual(response.status_code, 200)
        ...
        self.app.patch_json("/monitorings/{}".format(self.monitoring_id), {"data": {"status": "active"}}, status=200)
    ...
    def test_patch_to_cancelled_with_no_report(self):
        self.app.authorization = ("Basic", (self.sas_name, self.sas_pass))
        response = self.app.patch_json(
            "/monitorings/{}".format(self.monitoring_id), {"data": {"status": "cancelled"}}, status=422
        )
...
class ActiveMonitoringResourceTest(BaseWebTest):
    ...
    def test_patch_to_stopped(self):
        response = self.app.patch_json(
            "/monitorings/{}".format(self.monitoring_id),
            {"data": {"status": "stopped", "cancellation": {"description": "Whisper words of wisdom - let it be."}}},
        )
    ...
    def test_patch_to_stopped_with_no_report(self):
        response = self.app.patch_json(
            "/monitorings/{}".format(self.monitoring_id), {"data": {"status": "stopped"}}, status=422
        )
        self.assertEqual(("body", "cancellation"), next(get_errors_field_names(response, "This field is required.")))
...
class AddressedMonitoringResourceTest(BaseWebTest):
    ...
    def test_patch_add_cancellation(self):
        response = self.app.patch_json(
            "/monitorings/{}".format(self.monitoring_id),
            {"data": {"cancellation": {"description": "Whisper words of wisdom - let it be."}}},
        )
        ...
        with freeze_time(cancellation_date):
            response = self.app.patch_json(
                "/monitorings/{}".format(self.monitoring_id),
                {
                    "data": {
                        "status": "stopped",
                        "cancellation": {"description": "Whisper words of wisdom - let it be."},
                    }
                },
            )
            self.assertEqual(response.json["data"]["status"], "stopped")
...
@freeze_time("2025-11-01T12:00:00.000000+03:00")  # Saturday
class StoppedMonitoringWeekendStartTest(BaseWebTest):
    def setUp(self):
        super().setUp()
        self.create_monitoring()
        self.app.authorization = ("Basic", (self.sas_name, self.sas_pass))
    ...
    @unittest.skipIf(SANDBOX_MODE, "Skip test with accelerator")
    def test_patch_stopped_to_active_when_start_date_is_weekend(self):
        with freeze_time("2025-11-03T12:00:00+02:00"):
            response = self.app.patch_json(
                "/monitorings/{}".format(self.monitoring_id),
                {"data": {"status": "stopped", "cancellation": {"description": "test"}}},
            )
            self.assertEqual(response.json["data"]["status"], "stopped")

        with freeze_time("2025-11-05T12:00:00+02:00"):
            response = self.app.patch_json(
                "/monitorings/{}".format(self.monitoring_id),
                {"data": {"status": "active"}},
            )
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json["data"]["status"], "active")
        self.assertIn("endDate", response.json["data"]["monitoringPeriod"])
