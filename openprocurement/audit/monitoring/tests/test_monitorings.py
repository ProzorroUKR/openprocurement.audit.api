...
class MonitoringsEmptyListingResourceTest(BaseWebTest, DSWebTestMixin):
    ...
    def test_post_monitoring_sas_empty_body(self):
        ...
        self.assertEqual(("body", "data"), next(get_errors_field_names(response, "Data not available")))
    ...
    def test_post_monitoring_risk_bot(self):
        ...
        data = {
            ...
            "riskIndicatorsRegion": "Севастополь",
            "decision": {
        ...
        with mock.patch("openprocurement.audit.monitoring.utils.TendersClient") as mock_api_client:
            mock_api_client.return_value.get_tender.return_value = {"config": {"restricted": False}}
            response = self.app.post_json("/monitorings", {"data": data}, status=201)
            ...
            response = self.app.post_json(
                "/monitorings",
                {
                    "data": {
                        ...
                        "procuringStages": ["awarding", "contracting"],
                        "status": "draft",
                        "eliminationReport": {"description": "Report from the tender owner"},
                    }
                },
            )
    ...
    def test_pagination(self):
        ...
        for i in range(pages + 2):
            ...
            if i < pages:
                ...
                self.assertEqual(
                    [m["id"] for m in response.json["data"]], self.expected_ids[offset : offset + self.limit]
                )
                offset += self.limit
...
class DraftChangesFeedTestCase(BaseWebTest):
    ...
    def test_all_draft(self):
        ...
        self.assertEqual(
            set(e["id"] for e in response.json["data"]), set(self.expected_real_ids + self.expected_test_ids)
        )
...
class FeedVisibilityTestCase(BaseWebTest):
    ...
    def test_real_date_modified(self):
        url = "/monitorings"
        response = self.app.get(url)
        self.assertEqual(set(e["id"] for e in response.json["data"]), {self.active_id})
    ...
    def test_test_date_modified(self):
        ...
        response = self.app.get(url)
        self.assertEqual(set(e["id"] for e in response.json["data"]), {self.test_active_id})
    ...
    def test_all_date_modified(self):
        ...
        response = self.app.get(url)
        self.assertEqual(set(e["id"] for e in response.json["data"]), {self.active_id, self.test_active_id})
    ...
    def test_real_draft_date_modified(self):
        ...
        response = self.app.get(url)
        self.assertEqual(
            set(e["id"] for e in response.json["data"]), {self.active_id, self.draft_id, self.cancelled_id}
        )
