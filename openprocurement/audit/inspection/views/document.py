...
from openprocurement.audit.inspection.utils import apply_patch, op_resource, save_inspection
...
class InspectionsDocumentBaseResource(APIResource):
    ...
    @json_view(permission="edit_inspection", validators=(validate_file_upload,))
    def collection_post(self):
        ...
        if save_inspection(self.request):
            self.LOGGER.info(
                "Created inspection document {}".format(document.id),
                extra=context_unpack(
                    self.request, {"MESSAGE_ID": "inspection_document_create"}, {"DOCUMENT_ID": document.id}
                ),
            )
    ...
    @json_view(permission="edit_inspection", validators=(validate_file_update,))
    def put(self):
        ...
        if save_inspection(self.request):
            self.LOGGER.info(
                ...
                extra=context_unpack(
                    self.request, {"MESSAGE_ID": "inspection_document_put"}, {"DOCUMENT_ID": document.id}
                ),
            )
            return {"data": document.serialize("view")}

    @json_view(
        content_type="application/json", permission="edit_inspection", validators=(validate_patch_document_data,)
    )
    def patch(self):
        ...
        if apply_patch(self.request):
            self.LOGGER.info(
                "Updated inspection document {}".format(document.id),
                extra=context_unpack(
                    self.request, {"MESSAGE_ID": "inspection_document_patch"}, {"DOCUMENT_ID": document.id}
                ),
            )
