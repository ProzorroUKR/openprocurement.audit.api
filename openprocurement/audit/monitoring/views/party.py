...
from openprocurement.audit.monitoring.validation import validate_party_data, validate_patch_party_data


@op_resource(
    name="Monitoring Party",
...
class PartyResource(APIResource):
    @json_view(content_type="application/json", validators=(validate_party_data,))
    def collection_post(self):
        ...
        if save_monitoring(self.request):
            self.LOGGER.info(
                ...
                extra=context_unpack(self.request, {"MESSAGE_ID": "monitoring_party_create"}, {"party_id": party.id}),
            )
    ...
    @json_view(content_type="application/json", validators=(validate_patch_party_data,), permission="edit_party")
    def patch(self):
        """
        ...
        self.LOGGER.info(
            "Updated party {}".format(self.request.context.id),
