...
class PostResource(APIResource):
    @json_view(content_type="application/json", validators=(validate_post_data,), permission="create_post")
    def collection_post(self):
        ...
        if save_monitoring(self.request):
            self.LOGGER.info(
                "Created monitoring post {}".format(post.id),
                extra=context_unpack(self.request, {"MESSAGE_ID": "monitoring_post_create"}, {"POST_ID": post.id}),
            )
