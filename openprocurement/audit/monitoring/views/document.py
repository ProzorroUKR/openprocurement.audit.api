...
from openprocurement.audit.api.utils import context_unpack, get_file, upload_file
...
from openprocurement.audit.monitoring.utils import apply_patch, op_resource, save_monitoring, set_author
...
class MonitoringsDocumentBaseResource(APIResource):
    ...
    @json_view(permission="edit_monitoring", validators=(validate_file_upload,))
    def collection_post(self):
        ...
        if save_monitoring(self.request):
            self.LOGGER.info(
                "Created {} monitoring document {}".format(self.document_type, document.id),
                extra=context_unpack(
                    self.request, {"MESSAGE_ID": "monitoring_document_create"}, {"DOCUMENT_ID": document.id}
                ),
            )
    ...
    @json_view(permission="edit_monitoring", validators=(validate_file_update,))
    def put(self):
        """
        ...
        if save_monitoring(self.request):
            self.LOGGER.info(
                "Updated {} monitoring document {}".format(self.document_type, document.id),
                extra=context_unpack(
                    self.request, {"MESSAGE_ID": "monitoring_document_put"}, {"DOCUMENT_ID": document.id}
                ),
            )
            return {"data": document.serialize("view")}

    @json_view(
        content_type="application/json", permission="edit_monitoring", validators=(validate_patch_document_data,)
    )
    def patch(self):
        """
        Monitoring Document Update
        ...
        if apply_patch(self.request, update_context_date=True):
            self.LOGGER.info(
                "Updated {} monitoring document {}".format(self.document_type, document.id),
                extra=context_unpack(
                    self.request, {"MESSAGE_ID": "monitoring_document_patch"}, {"DOCUMENT_ID": document.id}
                ),
            )
...
@op_resource(
    name="Monitoring Cancellation Documents",
...
@op_resource(
    name="Monitoring Decision Documents",
...
@op_resource(
    name="Monitoring Conclusion Documents",
...
@op_resource(
    name="Monitoring Post Documents",
...
@op_resource(
    name="Monitoring Elimination Report Documents",
...
class MonitoringsDocumentEliminationResource(MonitoringsDocumentBaseResource):
    ...
    @json_view(permission="create_elimination_report", validators=(validate_file_upload,))
    def collection_post(self):
        return super(MonitoringsDocumentEliminationResource, self).collection_post()

    @json_view(permission="create_elimination_report", validators=(validate_patch_document_data,))
    def patch(self):
        return super(MonitoringsDocumentEliminationResource, self).patch()

    @json_view(permission="create_elimination_report", validators=(validate_file_update,))
    def put(self):
        return super(MonitoringsDocumentEliminationResource, self).put()


@op_resource(
    name="Monitoring Elimination Resolution Documents",
...
@op_resource(
    name="Monitoring Appeal Documents",
...
class AppealDocumentResource(MonitoringsDocumentBaseResource):
    ...
    @json_view(permission="create_appeal", validators=(validate_file_upload,))
    def collection_post(self):
        return super(AppealDocumentResource, self).collection_post()

    @json_view(permission="create_appeal", validators=(validate_patch_document_data,))
    def patch(self):
        return super(AppealDocumentResource, self).patch()

    @json_view(permission="create_appeal", validators=(validate_file_update,))
    def put(self):
        return super(AppealDocumentResource, self).put()


@op_resource(
    name="Monitoring LIABILITY Documents",
...
class LiabilityDocumentResource(MonitoringsDocumentBaseResource):
    ...
    @json_view(permission="edit_monitoring", validators=(validate_patch_document_data,))
    def patch(self):
