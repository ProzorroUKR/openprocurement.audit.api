...
class MonitoringPartyResourceTest(BaseWebTest, DSWebTestMixin):
    ...
    def test_party_create_required_fields(self):
        self.app.authorization = ("Basic", (self.sas_name, self.sas_pass))
        response = self.app.post_json("/monitorings/{}/parties".format(self.monitoring_id), {"data": {}}, status=422)
        ...
        response = self.app.patch_json(
            "/monitorings/{}/parties/{}".format(self.monitoring_id, party_id),
            {
                "data": {
                    ...
                    "contactPoint": {"telephone": "0449999999"},
                    "address": {"region": "Kharkov", "locality": "Kharkov"},
                }
            },
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_party_get_list(self):
        self.app.authorization = ("Basic", (self.sas_name, self.sas_pass))
        self.app.post_json("/monitorings/{}/parties".format(self.monitoring_id), {"data": self.initial_party})
