...
class MonitoringInspectionsResourceTest(BaseWebTest):
    ...
    def test_get_two(self):
        ...
        expected_two = {
            "dateCreated": "2018-01-01T11:00:50+02:00",
            "dateModified": "2018-01-01T11:00:50+02:00",
            "inspection_id": self.inspectionId,
            "id": self.inspection_id,
        }
        ...
        for uid in self.monitoring_ids:
            ...
            self.assertEqual(response.status, "200 OK")
            self.assertEqual(response.content_type, "application/json")
            self.assertEqual(response.json["data"], [expected_one, expected_two])
    ...
    def test_get_with_pagination(self):
        ...
        for uid in self.monitoring_ids:
            response = self.app.get("/monitorings/{}/inspections?limit=2&page=2".format(uid))
    ...
    def test_get_with_pagination_not_full_page(self):
        ...
        for uid in self.monitoring_ids:
            response = self.app.get("/monitorings/{}/inspections?limit=2&page=3".format(uid))
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.content_type, "application/json")
            self.assertEqual(response.json["total"], 5)
    ...
    def test_get_with_pagination_out_of_bounds_page(self):
        ...
        for uid in self.monitoring_ids:
            response = self.app.get("/monitorings/{}/inspections?limit=2&page=4".format(uid))
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.content_type, "application/json")
            self.assertEqual(response.json["total"], 5)
