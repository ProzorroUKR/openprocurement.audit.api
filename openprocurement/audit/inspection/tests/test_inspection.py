...
class InspectionResourceTest(BaseWebTest):
    ...
    def test_patch_forbidden(self):
        ...
        self.app.patch_json(
            "/inspections/{}".format(self.inspection_id), {"description": "I regretted my decision"}, status=403
        )
    ...
    def test_patch(self):
        ...
        self.app.authorization = ("Basic", (self.sas_name, self.sas_pass))
        request_data = {"description": "I regretted my decision", "monitoring_ids": ["5" * 32, "3" * 32]}

        modified_date = "2018-01-02T13:30:00+02:00"
        with freeze_time(modified_date):
            response = self.app.patch_json("/inspections/{}".format(self.inspection_id), {"data": request_data})
    ...
    def test_patch_validation_error(self):
        ...
        self.app.authorization = ("Basic", (self.sas_name, self.sas_pass))
        request_data = {"description": 12.5, "monitoring_ids": "something else"}
        response = self.app.patch_json(
            "/inspections/{}".format(self.inspection_id),
            {"data": request_data},
            status=422,
        ...
        self.assertEqual(
            ...
            [
                {"location": "body", "name": "monitoring_ids", "description": ["Hash value is wrong length."]},
                {"location": "body", "name": "description", "description": ["Couldn't interpret '12.5' as string."]},
            ],
        )
