...
from dateorro import calc_datetime, calc_next_working_datetime, calc_normalized_datetime, calc_working_datetime
...
from openprocurement.audit.api.constants import MONITORING_TIME, WORKING_DAYS
...
from openprocurement.audit.monitoring.models import Monitoring, Period
        ...
        with handle_store_exceptions(request):
            ...
            LOGGER.info(
                "Saved monitoring {}: dateModified {} -> {}".format(
                    monitoring.id, old_date_modified, monitoring.dateModified.isoformat()
                ),
...
def add_revision(request, item, changes):
    revision_data = {"author": request.authenticated_userid, "changes": changes, "rev": item.rev}
