...
class MonitoringCredentialsResourceTest(BaseWebTest):
    ...
    def test_credentials_no_access_token(self):
        ...
        response = self.app.patch_json("/monitorings/{}/credentials".format(self.monitoring_id), status=403)
    ...
    def test_credentials_query_param_wrong_access_token(self, client_class_mock):
        ...
        self.app.authorization = ("Basic", (self.broker_name, self.broker_pass))
        response = self.app.patch_json(
            "/monitorings/{}/credentials?acc_token={}".format(self.monitoring_id, "wrong_token"), status=403
        )
    ...
    def test_credentials_no_tender(self, client_class_mock):
        ...
        self.app.authorization = ("Basic", (self.broker_name, self.broker_pass))
        response = self.app.patch_json(
            "/monitorings/{}/credentials?acc_token={}".format(self.monitoring_id, "tender_token"), status=403
        )
        self.assertEqual(response.content_type, "application/json")
        self.assertEqual(
            ("body", "data"), next(get_errors_field_names(response, "Tender {} not found".format("f" * 32)))
        )
    ...
    def test_credentials_tender_error(self, client_class_mock):
        ...
        self.app.authorization = ("Basic", (self.broker_name, self.broker_pass))
        response = self.app.patch_json(
            "/monitorings/{}/credentials?acc_token={}".format(self.monitoring_id, "tender_token"), status=555
        )
        self.assertEqual(response.content_type, "application/json")
        self.assertEqual(
            response.json,
            {
                "status": "error",
                "errors": [{"location": "body", "name": "data", "description": "Unsuccessful tender request"}],
            },
        )
