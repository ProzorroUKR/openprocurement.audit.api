...
class AuthTest(unittest.TestCase):
    ...
    def test_unauthenticated_userid(self):
        ...
        request.headers['Authorization'] = 'Basic %s' % base64.b64encode(b'chrisr:password').decode('ascii')
    ...
    def test_unauthenticated_userid_invalid_payload(self):
        request = testing.DummyRequest()
        request.headers['Authorization'] = 'Basic %s' % base64.b64encode(b'chrisrpassword').decode('ascii')
        policy = self._makeOne(None)
        self.assertEqual(policy.unauthenticated_userid(request), None)
