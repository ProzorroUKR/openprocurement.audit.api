...
class MonitoringDecisionResourceTest(BaseWebTest, DSWebTestMixin):
    ...
    def test_fail_empty(self):
        self.app.patch_json("/monitorings/{}".format(self.monitoring_id), {"data": {"decision": {}}}, status=422)

    def test_success_minimal(self):
        decision_date = datetime.now(TZ) - timedelta(days=2)
        response = self.app.patch_json(
            "/monitorings/{}".format(self.monitoring_id),
            {"data": {"decision": {"description": "text", "date": decision_date.isoformat()}}},
        )
        ...
        response = self.app.put_json(
            "/monitorings/{}/decision/documents/{}".format(self.monitoring_id, doc_data["id"]),
            {"data": request_data},
        )
    ...
    def test_success_full(self):
        ...
        response = self.app.patch_json("/monitorings/{}".format(self.monitoring_id), {"data": {"decision": decision}})
        self.assertEqual(response.status_code, 200)
    ...
    def test_visibility(self):
        self.app.patch_json(
            "/monitorings/{}".format(self.monitoring_id),
            {"data": {"decision": {"description": "text", "date": datetime.now(TZ).isoformat()}}},
        )
        ...
        self.app.authorization = ("Basic", (self.sas_name, self.sas_pass))
        self.app.patch_json("/monitorings/{}".format(self.monitoring_id), {"data": {"status": ACTIVE_STATUS}})
    ...
    def test_decision_endpoint(self):
        ...
        self.assertEqual(set(response.json["data"].keys()), {"date", "dateCreated", "description", "documents"})
        ...
        response = self.app.get("/monitorings/{}/decision".format(self.monitoring_id))
        self.assertEqual(
            set(response.json["data"].keys()), {"date", "dateCreated", "description", "documents", "datePublished"}
        )
    ...
    def test_decision_party_create(self):
        ...
        response = self.app.patch_json(
            ...
            {
                "data": {
                    "decision": {"description": "text", "date": decision_date.isoformat(), "relatedParty": party_id}
                }
            },
        )
        self.assertEqual(response.status_code, 200)
